...
if active_page == "AI Re-Ranking":
    ...
    with st.expander(
        f"View the {len(recs)} collaborative-filtering candidates sent to Gemini",
        expanded=False,
    ):
        st.caption(
            "These are the original recommendations before Gemini changes their "
            "order. The graph shows each book's predicted rating from the "
            "collaborative-filtering model; it is not an AI score."
        )
        preview_left, preview_right = st.columns([1, 1])
        with preview_left:
            st.markdown(
                "<div class='section-title'>Original CF Candidate List</div>",
                unsafe_allow_html=True,
            )
            for rank, row in enumerate(recs[:5], start=1):
                render_book_card(row, rank)
        with preview_right:
            if PLOTLY_AVAILABLE:
                ...
                fig = px.bar(
                    ...
                    color="Predicted",
                    color_continuous_scale=["#00e054", "#40bcf4", "#ff8000"],
                    title="Collaborative-Filtering Scores Before Gemini",
                    labels={
                        "Predicted": "Predicted rating from CF model",
                        "Book": "Book",
                    },
                )
                fig.update_layout(coloraxis_showscale=False)
...
if active_page == "Dataset & Evaluation":
    ...
    with overview_tab:
        ...
        with d6:
            stat_box(overview["Avg ratings per book"], "Avg ratings per rated book")
        with d7:
        ...
        st.markdown(
            ...
            <div class="model-note">
                ...
                filtering until they receive rating history.
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with diagnostics_tab:
        ...
        st.caption(
            "The 98.50% sparsity value uses the 1,192 users x 9,229 books that "
            "have at least one rating. The remaining 735 catalog books have no "
        ...
        st.markdown(
            ...
            **Book cold start:** The 735 books with no ratings cannot be recommended by
            collaborative filtering. Another 2,813 rated books have fewer than five ratings,
